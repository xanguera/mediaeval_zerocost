...
if [[ ! -z $phone_symbol_table ]]; then
  ...
  n1=`perl -nle'print $_ if $_ !~ /^#[0-9]+/' $phone_symbol_table | cut -d' ' -f1 | sort -u | wc -l`
	#n2=`cat $phone_symbol_table | grep -v -P "^#[0-9]+$" | cut -d' ' -f1 | sed 's/_[BIES]$//g' | sort -u | wc -l`
	n2=`perl -nle'print $_ if $_ !~ /^#[0-9]+/' $phone_symbol_table | cut -d' ' -f1 | sed 's/_[BIES]$//g' | sort -u | wc -l`
  ...
  # Checks if the phone sets match.
	# OSX AWK does not allow (XX|), I need to do it in two regexps
  # BEGIN { while ((getline < f) > 0) { sub(/((_[BEIS])|) [0-9]+$/, "", $0); phones[$0] = 1; }}
  cat $srcdir/{,non}silence_phones.txt | awk -v f=$phone_symbol_table '
  BEGIN { while ((getline < f) > 0) { sub(/_[BEIS] [0-9]+$/, "", $0); sub(/ [0-9]+$/, "", $0); phones[$0] = 1; }}
